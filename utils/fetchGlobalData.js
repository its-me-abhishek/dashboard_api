...
async function fetchGlobalCryptoData(currency = 'USD') {
    try {
        const response = await axios.get(`https://api.alternative.me/v2/global/?convert=${currency}`);

        if (response.data && response.data.data) {
            return {
                marketCap: response.data.data.quotes[currency].total_market_cap,
                marketCapChange: response.data.data.quotes[currency].total_market_cap_yesterday_percentage_change,
